@echo off
REM Hermes Tray Windows 构建测试脚本
...
if errorlevel 1 (
    ...
    if errorlevel 1 (echo [通过] Inno Setup) else (echo [通过] Inno Setup)
)
...
if "%ERROR_NODE%"=="1" (
    ...
    if errorlevel 1 (
        ...
        if exist "node_modules\@tauri-apps\cli" (
            echo [通过] @tauri-apps/cli 已安装
        ) else (
            echo [失败] @tauri-apps/cli 缺失
        )
...
echo.
echo --- 执行统计 ---
echo 环境检查: %ERROR_NODE%%ERROR_NPM%%ERROR_RUST%
echo npm 依赖: 已完成
echo 前端构建: 已完成
echo Tauri构建: 已完成
echo 安装程序: 已完成
...
if exist "installer\*.exe" (
    for %%A in ("installer\*.exe") do (
        echo   Installer: %%~fA (%%~zA bytes) >> build-test-summary.txt
    )
)
echo. >> build-test-summary.txt
echo Prerequisites: >> build-test-summary.txt
echo   Node.js: [%ERROR_NODE%] Inno Setup: [%SKIP_ISCC%] >> build-test-summary.txt
echo. >> build-test-summary.txt
